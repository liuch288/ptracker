...
@app.command("add")
def add_trade(
    trade_type: str = typer.Argument(..., help="Trade type: buy or sell"),
    asset: str = typer.Argument(..., help="Asset code (e.g., AAPL, 0700.HK)"),
    quantity: float = typer.Argument(..., help="Quantity (positive number)"),
    price: float = typer.Argument(..., help="Price per unit"),
    currency: str = typer.Option(..., "--currency", "-c", help="Currency code (e.g., USD, HKD)"),
    action: str = typer.Option("open", "--action", "-a", help="Action: open or close"),
    ...
    if trade_type.lower() not in ['buy', 'sell']:
        console.print(f"[red]Error: Invalid trade type '{trade_type}'. Must be 'buy' or 'sell'.[/red]")
        raise typer.Exit(1)
    ...
    if action.lower() not in ['open', 'close']:
        console.print(f"[red]Error: Invalid action '{action}'. Must be 'open' or 'close'.[/red]")
        raise typer.Exit(1)
    ...
    if trade_type == "buy" and action == "open" and direction == "long":
...
def list_trades(
    ...
    trade_type: Optional[str] = typer.Option(None, "--type", "-t", help="Filter by type (buy/sell)"),
    action: Optional[str] = typer.Option(None, "--action", help="Filter by action (open/close)"),
